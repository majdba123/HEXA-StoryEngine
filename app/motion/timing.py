...
class MotionTimingPolicy:
    ...
    def window(
        ...
        *,
        ...
        if primary:
            start = beat.start
            desired_end = max(start + duration, audio_start + min(0.18, spoken_duration * 0.12))
            end = min(beat.end - 0.03, desired_end)
        else:
        ...
        end = max(start + min(self._MIN_DURATION, visual_duration), end)
