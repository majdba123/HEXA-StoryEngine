...
class StoryPlanner:
    ...
    def _select_assets(scene_assets: list[VisualAsset]) -> list[VisualAsset]:
        """Keep a bounded but expressive set of strict cutout objects for a beat.

        Vision may expose many tiny isolated highlights. Story keeps the dominant
        objects plus enough independent cues (arrows, lamps, badges, signs) to support
        reference-style storytelling without turning a short spoken phrase into a
        machine-gun sequence of entrances. Motion decides which of these receive a
        strong entrance and which arrive as quiet context.
        """
        if len(scene_assets) <= 8:
            return scene_assets
        return scene_assets[:8]
