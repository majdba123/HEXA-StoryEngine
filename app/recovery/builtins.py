...
BUILTIN_ISSUES = [
    ...
    KnownIssue(code="MOTION_TOO_FAST", description="An entrance is too fast for readable storytelling", affected_stage="motion", handler="rebuild_motion", status=RecoveryStatus.proven),
    KnownIssue(code="INSUFFICIENT_VISUAL_HOLD", description="A beat lacks enough settled reading time", affected_stage="motion", handler="rebuild_motion", status=RecoveryStatus.proven),
    KnownIssue(code="MOTION_OVERLOAD", description="A spoken beat contains too many strong motion events", affected_stage="motion", handler="rebuild_motion", status=RecoveryStatus.proven),
